...
const ImageResizeContent = () => {
  ...
  const resizeImage = useCallback(async () => {
    ...
    try {
      ...
      setResizedPreview(newPreviewUrl);
  ...
  }, [originalImage, originalPreview, settings.maxWidth, settings.maxHeight, settings.quality, settings.format]);
  ...
  const handleDownload = useCallback(() => {
    ...
    // ファイル名を生成（元のファイル名 + _resized + 拡張子）
    const originalFileName = originalImage?.name || 'image';
    const fileNameWithoutExt = originalFileName.replace(/\.[^/.]+$/, '');
    const extension = settings.format === 'jpeg' ? 'jpg' : settings.format;
    const newFileName = `${fileNameWithoutExt}_resized.${extension}`;
  ...
  }, [resizedImage, originalImage, settings.format]);
